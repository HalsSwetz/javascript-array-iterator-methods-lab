...
let veryOldInventors = inventors.filter((veryOldInventor) => {
    if (veryOldInventor.year < 1599 && veryOldInventor.year > 1500) return veryOldInventor;
});
...
const inventorNamedAda = inventors.find((inventorAda) => {
    return inventorAda.first === 'Ada';
})
...
const isAdultPresent = devs.some((dev) => {
    return 2015 - dev.year >= 18;
})

//i might be missing the point of the original null value,
//or is this array.some() that correctly answers the question ok?
...
const isEveryone19OrOlder = devs.every((dev) => {
    if(2025 - dev.year >= 19) return true;
})

//same as above, not sure if I should take into consideration the 
//let "" = null above, or simply write something with array.every
...
const commentById = comments.find((comment) => {
    if (comment.id === 823423) return comment.text;
});
